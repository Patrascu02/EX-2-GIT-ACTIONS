...
1.Creaza un branch nou
* **1.Creaza un fisier .txt care sa contina urm informatii:
  * run_id
  * actor
  * repository
  * te vei fol. de var. puse la dispozitie de catre github,github.run_id,github.actor,github.repository)
1.da commit la fisier si push
1.sa se faca un PR fol. gh pr create din bracnchul tau catre main(GitHub CLI | Take Gitub to the command line).Sa se adauge label-ul "github-automation" la acest PR , sa se adauge un body si un title .Daca nu exista acel label poti sa il creezi manual din interfata inainte sa rulezi workflow-ul
